...
impl DeferredBatch {
    ...
    pub async fn write(self) -> Result<(), TypedStoreError> {
        ...
        let prev_id_or_batch = {
            ...
            batcher
                .write_batch
                .add_deferred_batch_operations(operations)?;
            batcher.write_batch_size += 1;
            batcher.senders.push(tx);

            if batcher.write_batch_size == BatchBatcher::MAX_BATCH_SIZE {
                let new_batch = DBBatch::new(&rocksdb, &db_metrics, &write_sample_interval);
                Either::Right(BatchBatcher::take_and_replace(batcher, new_batch))
            } else {
        ...
        let batcher = {
            ...
            if prev_id == batcher.id {
                let new_batch = DBBatch::new(&rocksdb, &db_metrics, &write_sample_interval);
                Some(BatchBatcher::take_and_replace(batcher, new_batch))
            } else {
                None
            }
        };
...
struct BatchBatcher {
    ...
    senders: Vec<oneshot::Sender<Result<(), TypedStoreError>>>,
    id: u64,
}
...
impl BatchBatcher {
    ...
    fn new(write_batch: DBBatch) -> Self {
        Self {
            write_batch,
            write_batch_size: 0,
            senders: Vec::new(),
    ...
    fn write(self) {
        let result = self.write_batch.write();
    ...
    fn take_and_replace(mut guard: MutexGuard<'_, BatchBatcher>, new_batch: DBBatch) -> Self {
        let this = &mut *guard;
        let mut new = BatchBatcher::new(new_batch);
        new.id = this.id + 1;
...
impl<K, V> DBMap<K, V> {
    pub(crate) fn new(db: Arc<RocksDB>, opts: &ReadWriteOptions, opt_cf: &str) -> Self {
        ...
        DBMap {
            rocksdb: db.clone(),
            batch_batcher: Arc::new(Mutex::new(BatchBatcher::new(DBBatch::new(
                &db,
                &db_metrics_cloned,
                &db.write_sampling_interval(),
            )))),
            opts: opts.clone(),
    ...
    V: Serialize + DeserializeOwned,
{
    ...
    fn insert(&self, key: &K, value: &V) -> Result<(), TypedStoreError> {
        ...
        let mut opts = self.opts.writeopts();
        opts.disable_wal(true);
        self.rocksdb
            .put_cf(&self.cf(), &key_buf, &value_buf, &opts)?;
        Ok(())
    }
    ...
    fn remove(&self, key: &K) -> Result<(), TypedStoreError> {
        ...
        let mut opts = self.opts.writeopts();
        opts.disable_wal(true);
        self.rocksdb.delete_cf(&self.cf(), key_buf, &opts)?;
        if report_metrics.is_some() {
            self.db_metrics
                .op_metrics
                .rocksdb_deletes
